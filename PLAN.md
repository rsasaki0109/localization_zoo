...
| Item | Value |
|------|-------|
| Branch | `feat/unossed-papers-31` |
| 実装済み from-paper 論文数 | **31 本** (下記 §00.4) |
| `docs/methods.json` | **71 手法** (campaign 開始前 ~40 → 71) |
| 最新コミット | KC-LO (31本目) commit 済み |
| 直近完了 | **KC-LO (31本目)** — seq00 0.842% / seq07 **0.514%** (seq07 RPE 全体トップ)、**positive**: 両 seq で KISS-ICP の drift を上回る (ただし ~1.4 FPS と低速) |
...
- distinct な古典機構は **ほぼ枯渇**。saturated: ground/degeneracy/voxelmap/feature-weighting/
  distribution-factor/motion-constraint/robust-M-estimator/robust-LTS-trimming(TrICP)/spectral/
  gmm-em/correntropy/implicit-surface(IMLS)/correlation-entropy(KC, ただし positive・低速)。direct/range-image は発散。公開コード有で除外:
  symmetric-ICP(PCL), MAD-ICP, Anderson-ICP, Fast-Robust-ICP, Surfel-LIO, DELO,
  Retrospective Map Refinement, 最適輸送系 (DELO 等は deep+OSS)。
