...
>
> 2026-06-09 セッション (続き): **PL-LOAM/VLOM KITTI Raw RGB 本評価**、
> **InTEn-LOAM TVF/DOR/mapping 拡張**、**MulRan/NCLT cross-dataset 再評価** を実施。
> RGB eval は honest negative (~99% drift, 実 PNG でも改善なし)。InTEn mapping で
> seq07 prefix 56.5% drift。**Cross-dataset 再評価完了**: MulRan 120 + NCLT 600。
...
> - **37 本目 VLOM** (arXiv:2304.08978) **完了**: 単眼スケール補正 + visual bootstrap A-LOAM。
>   seq00 **91.5%** / seq07 **153.9%** drift (honest negative、疑似画像制約)。§00.53 参照。
> - **新規 web サーベイ実施済み** (IMU dead-reckoning / LiDAR-visual / intensity-LiDAR の
>   3 カテゴリ)。Intensity は完了。IMU カテゴリ 3/3 完了 → §00.6 残候補参照。
...
| 実装済み from-paper 論文数 | **40 本** (§00.48–§00.56) |
| `docs/methods.json` | **80 手法** |
| 直近完了 | **NN-ZUPT (40本目)** — CNN 停止検出 + ZUPT; cross-IMU honest negative |
| その前 | **NHC-Net (39本目)** — VMSC CNN + adaptive NHC; cross-IMU honest negative |
| その前 | **OdoNet (38本目)** — CNN 疑似オドメータ + INS/NHC; cross-IMU honest negative |
| その前 | **VLOM (37本目)** — seq00 **91.5%** / seq07 **153.9%** (honest negative) |
| その前 | **ICPSC-LO (36本目)** — seq00 **0.912%** / seq07 **0.660%** |
