...
>
> 2026-06-08〜09 セッションで追加された動き (詳細は §00.05 / §00.48–§00.53 / §00.6):
...
>   KITTI で **positive** (drift -18〜20%)。
> - **33/34 本目 PL-LOAM / InTEn-LOAM** を `main` に push (commit `a8c3a75` / `06d6751`)。
>   いずれも honest negative (~117–143% / ~53–67% drift)。
> - **I-LOAM mapping 統合** (2026-06-09): `aloam::LaserMapping` を追加 (デフォルト ON)。
>   seq00 **0.899%** / seq07 **0.575%** drift → from-paper leaderboard 同列 (A-LOAM 水準)。
> - **35/36 本目 MCGICP-LO / ICPSC-LO** (commit `9184524`): Intensity カテゴリ shortlist 完了。
>   seq00 **0.940% / 0.912%**、seq07 **0.774% / 0.660%** — いずれも mid-pack (GMM-LO 帯)。
> - **37 本目 VLOM** (arXiv:2304.08978) 実装中: 単眼スケール補正 + visual bootstrap A-LOAM。
>   seq07 **153.9%** drift (honest negative、疑似画像制約)。§00.53 参照。
> - **新規 web サーベイ実施済み** (IMU dead-reckoning / LiDAR-visual / intensity-LiDAR の
>   3 カテゴリ)。Intensity は完了。次候補は §00.6 (OdoNet 等)。
...
| 実装済み from-paper 論文数 | **37 本** (§00.48–§00.53; VLOM はベンチ・README 仕上げ中) |
| `docs/methods.json` | **75 手法** (campaign 開始前 ~40 → 75; VLOM 追加済み) |
| main HEAD | `9184524` (papers 35–36 MCGICP/ICPSC) → 次 commit で VLOM (paper 37) |
| 直近 WIP | **VLOM (37本目)** — 単眼スケール補正 + visual-bootstrapped A-LOAM。seq07 **153.9%** / ATE 439 m (honest negative) |
| 直近完了 | **ICPSC-LO (36本目)** — 円筒 ICPSC + 適応 geom/intensity scan-to-map。seq00 **0.912%** / seq07 **0.660%** |
| その前 | **MCGICP-LO (35本目)** — 4D 共分散 GICP。seq00 **0.940%** / seq07 **0.774%** drift |
| その前 | **InTEn-LOAM (34本目)** — honest negative (~53–67% drift) |
| その前 | **PL-LOAM (33本目)** — honest negative (~117–143% drift, 疑似画像) |
| 直近 PR | **#11** / **#12** / **#13** — いずれも CI green でマージ済み |

直近コミット列 (main、campaign 部分):

```
9184524 Add papers 35-36: MCGICP-LO and ICPSC-LO intensity odometry.
d45304f Add I-LOAM LaserMapping integration for competitive KITTI drift.
06d6751 Add paper 34: InTEn-LOAM (cylindrical intensity LO, RS 2022/23)
a8c3a75 Add paper 33: PL-LOAM (LiDAR-visual point+line VO, ICRA 2020)
b02612c Merge PR #13 (I-LOAM, paper 32)
...（PR#11 に IMLS-SLAM/TrICP-LO/KC-LO + 50-star polish を内包）
```
...
結果 (honest negative): seq00 **143.2%** drift (ATE 3016 m)、seq07 **116.9%** (ATE 271 m)。

### 00.50 InTEn-LOAM (34本目, 最新完了)
...
を Ceres で joint 最適化。**Temporal DOR と mapping は後回し** (PLAN どおり)。
`test_inten_loam` 5/5 pass、`--inten-loam-no-intensity` ablation あり。
結果 (honest negative): seq00 **52.7%** drift (ATE 1459 m)、seq07 **67.5%** (ATE 448 m)。
seq07 強度 OFF は **66.9%** / ATE 373 m → 強度経路は near-neutral。
...
`--vlom-no-scale` あり。
結果 (honest negative, 疑似画像): seq07 **153.9%** drift (ATE 439 m, ~2.4 FPS)。
mean_scale≈1.003、bootstrap 172/1100 frames。PL-LOAM (117%) より悪化 — 視覚 bootstrap が
後半で A-LOAM を攪乱した可能性。seq00 ベンチは実行中/未記録。
**未完了**: seq00 ベンチ JSON、README leaderboard 行は honest-negative のため省略可、
`aloam` に `setMotionPrior` API 追加 (VLOM 用)。
...
### 00.6 次の一手 (37本目仕上げ後) + 新規サーベイ shortlist (2026-06-09 更新)
...
- ✅ **InTEn-LOAM** (RS 2022/23) — 完了 (§00.50)。scan-to-scan のみで ~53–67% drift;
  強度 ablation は near-neutral (DOR/mapping 省略が主因)。
...
- ✅ **PL-LOAM** (ICRA 2020) — 完了 (§00.49)。Odometry ベンチは疑似画像のみ → ~117–143% drift。
- 🔄 **VLOM** (arXiv:2304.08978) — 実装中 (§00.53)。scale correction + visual bootstrap A-LOAM。
  疑似画像 eval で seq07 **153.9%** (honest negative)。KITTI Raw RGB 本評価は将来 work。
...
**OdoNet** (IMU 学習、着手前に方式合意) → VLOM/PL-LOAM KITTI Raw RGB 本評価 →
InTEn-LOAM DOR/mapping。Intensity shortlist (I-LOAM / InTEn / MCGICP / ICPSC) は **完了**。
LiDAR-visual は VLOM 仕上げ (seq00 ベンチ、ablation) 後に枯渇 → IMU カテゴリへ。
代替: 既存手法の cross-dataset 再評価 (MulRan/NCLT)。サーベイ全文は会話ログ参照。
