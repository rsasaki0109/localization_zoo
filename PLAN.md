...
>
> 2026-06-08〜09 セッションで追加された動き (詳細は §00.05 / §00.48 / §00.6):
...
>   README leaderboard 行追加、ablation は `--i-loam-no-mapping` で scan-to-scan 分離。
...
| 実装済み from-paper 論文数 | **36 本** (§00.48–§00.52) |
| `docs/methods.json` | **74 手法** (campaign 開始前 ~40 → 74) |
| main HEAD | `06d6751` (paper 34 InTEn-LOAM) → 次 commit で I-LOAM mapping |
| 直近完了 | **ICPSC-LO (36本目)** — 円筒 ICPSC + 適応 geom/intensity scan-to-map。seq00 **0.912%** / seq07 **0.660%** (MCGICP 上回る) |
| その前 | **MCGICP-LO (35本目)** — 4D 共分散 GICP。seq00 **0.940%** / seq07 **0.774%** drift |
| 直前完了 | **InTEn-LOAM (34本目)** — 円筒 range+intensity + B-spline 強度登録 (DOR/mapping 省略)。seq00 **52.7%** / seq07 **67.5%** drift (honest negative) |
| その前 | **PL-LOAM (33本目)** — LiDAR 投影疑似画像上の点線 VO。seq00 **143%** / seq07 **117%** drift (RGB 無し honest negative) |
...
### 00.6 次の一手 (36本目完了後) + 新規サーベイ shortlist (2026-06-08)
...
**OdoNet** (IMU 学習、着手前に方式合意) → PL-LOAM KITTI Raw RGB 本評価 → InTEn-LOAM DOR/mapping。
I-LOAM mapping 統合は完了 (seq07 0.575%, seq00 0.899%)。
