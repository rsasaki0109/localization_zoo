...
| Item | Value |
|------|-------|
| Branch | `feat/unossed-papers-29` |
| 実装済み from-paper 論文数 | **29 本** (下記 §00.4) |
| `docs/methods.json` | **69 手法** (campaign 開始前 ~40 → 69) |
| 最新コミット | IMLS-SLAM (29本目) commit 済み (`9acbc9a` + 評価 follow-up) |
| 直近完了 | **IMLS-SLAM (29本目)** — seq00 1.000% / seq07 0.700%、honest negative (KISS-ICP 比 0.872/0.618 に僅差で劣) |
...
- distinct な古典機構は **かなり枯渇**。saturated: ground/degeneracy/voxelmap/feature-weighting/
  ...
  implicit-surface(IMLS)。direct/range-image は発散。公開コード有で除外: symmetric-ICP(PCL),
  MAD-ICP, Anderson-ICP, Fast-Robust-ICP, Surfel-LIO, DELO, Retrospective Map Refinement。
