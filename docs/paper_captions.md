...
_Generated at 2026-04-06T04:34:17+00:00 by `evaluation/scripts/export_paper_assets.py`._

## Table Caption

Table X. Current default variants selected from the experiment-driven benchmark contract.
The table summarizes 36 ready defaults across 15 GT-backed and 21 reference-based problem instances.
...
In the current GT-backed subset, the fastest default is `litamin2:fast_icp_only_half_threads` at 23.5 FPS, while the lowest ATE is `ndt:fast_coarse_map` at 0.005 m.

## Core Figure Caption

Figure Y. Manuscript-facing core defaults, choosing one representative default per method family.
The core set currently contains 6 method families and keeps `reference-based` outputs visually distinct from `gt-backed` outputs.
