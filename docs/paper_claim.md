...
The `pcd_dogfooding --summary-json` contract allows adding new variants and new benchmark windows without branching the evaluation runner. The current index tracks **256** ready problems, **1** blocked manifest, and **1** skipped manifest across **27** active selectors, all driven through `run_experiment_matrix.py` / `refresh_study_docs.py`.
...
|---------------|---------------|
| `experiments/results/index.json` | **256** ready + **1** blocked + **1** skipped problems; per-problem defaults |
