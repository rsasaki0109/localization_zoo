...
- A full artifact that currently wires **27 active selectors** and **262 indexed problems** (**260 ready**, **1 blocked**, **1 skipped**) across Istanbul, HDL-400 reference windows, public ROS1 HDL-400 synthetic-time windows, MCD, and KITTI Raw. The manuscript-facing core claim can still stay centered on the five families that share the same twelve-window grid.
...
- The current **262-problem / 27-selector** artifact demonstrates that default instability is **measurable wherever we grant equal window coverage**, not a corner case.
...
- Complete tables for **262** problem lines in the index (**260** ready + **1** blocked + **1** skipped) and per-variant rows inside each `experiments/results/*_matrix.json`.
