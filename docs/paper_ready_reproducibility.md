...
It freezes three methods for now: I-LOAM and KC-LO as T0 evidence candidates,
plus M-GCLO as T1+ evidence. The bundle records the seq00/07 main result rows,
paired-ablation summaries, raw artifact paths, and relative reproduce commands.
It is intentionally not the final 8-12 method manuscript table yet; Quadric-LO,
RF-LIO/ID-LIO, and the M-GCLO non-flat check remain open.
...
5. **Extend the frozen bundle** from the current 3-method seed to the final
