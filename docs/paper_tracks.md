...
Current coverage: `36` ready problems and `1` blocked problems.
...
- The stable benchmark contract now covers 36 ready problems under one CLI and one summary JSON interface.
...
- Current defaults already show non-trivial trade-offs, such as `LiTAMIN2=paper_icp_only_half_threads` at 105.6 FPS and `CT-LIO=seed_only_fast` at 0.412 m ATE on the public HDL-400 reference window.

### Gaps

- LiDAR-only methods now cover 7 repository-stored open sequences across 4 public dataset families, but external validity still rests on only 4 families.
