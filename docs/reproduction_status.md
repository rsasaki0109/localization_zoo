...
_Generated at 2026-05-17T22:25:14+00:00 by `evaluation/scripts/generate_reproduction_status.py`._
...
|--------|-------------|------------|---------------|--------------------|--------------|-----------|
| LiTAMIN2 | `approximately_reproduced` | Paper-oriented front-end reimplementation | Approximate reproduction across KITTI Odometry 00/02/05/07/08 (paper range) | Same metric, ~1.35x ratio, paper range | Stable ~1.35x ratio remaining vs paper after gap-closing tune (voxel 1.0 + max_iter 12). Correspondence pruning is now sweepable, and a radius-1 plus 1.5 m gate smoke run fixes a seq02 early-window failure, but it is not a uniform win on seq05/07/08. Remaining gaps likely also involve voxel covariance regularization, refresh_interval, and the missing coarse-to-fine schedule. Also sequences 01/03/04/06/09/10 not yet exported. | Export and run KITTI Odometry 01/03/04/06/09/10 for full paper coverage. Run full-sequence LiTAMIN2 correspondence sweeps (`--litamin2-correspondence-search-radius`, `--litamin2-max-correspondence-distance`) before deciding whether to promote a guarded profile; alternative: side-by-side with the upstream LiTAMIN2 reference implementation to identify which internal step introduces the ceiling. |
...
- **Current claim**: Approximate reproduction across KITTI Odometry 00/02/05/07/08. After the 2026-05-18 Tr-frame-correction work and gap-closing tuning (--ct-icp-dense-profile + --ct-icp-ceres-max-iterations 6 + --ct-icp-max-frames-in-map 20), CT-ICP pure odometry runs in the 1.28-3.35 % RPE range across KITTI Odometry 00/02/05/07/08 against paper's 0.30-0.75 %. As of 2026-06-12, that robust KITTI tune is the pcd_dogfooding default; --ct-icp-fast-profile keeps the old lightweight runtime profile available. Geometric-mean ratio 4.58x (was 5.44x with stock defaults). Gap is sticky under parameter tuning - the parameter-only ceiling is ~4.5x.
