...
|--------|-------------|------------|---------------|--------------------|--------------|-----------|
| LiTAMIN2 | `approximately_reproduced` | Paper-oriented front-end reimplementation | Approximate reproduction across KITTI Odometry 00/02/05/07/08 (paper range) | Same metric, ~1.35x ratio, paper range | Stable ~1.35x ratio remaining vs paper after gap-closing tune (voxel 1.0 + max_iter 12). Likely tied to dogfooding-specific implementation choices (correspondence pruning, voxel covariance regularization, refresh_interval) rather than profile parameters. Also sequences 01/03/04/06/09/10 not yet exported. | Export and run KITTI Odometry 01/03/04/06/09/10 for full paper coverage. Investigate the remaining ~1.35x ceiling by sweeping correspondence pruning and covariance regularization parameters; alternative: side-by-side with the upstream LiTAMIN2 reference implementation to identify which internal step introduces the ceiling. |
