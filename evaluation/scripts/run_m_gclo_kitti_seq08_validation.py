#!/usr/bin/env python3
"""Run M-GCLO ground on/off validation on public KITTI Odometry seq08."""
...
BENCH_DIR = REPO_ROOT / "docs" / "benchmarks" / "kitti_seq08_public"
PCD_DIR = REPO_ROOT / "dogfooding_results" / "kitti_seq_08_full"
GT_CSV = REPO_ROOT / "experiments" / "reference_data" / "kitti_seq_08_full_gt.csv"
...
def build_summary(on_json: Path, off_json: Path, output: Path) -> None:
    ...
    summary = {
        "schema_version": 1,
        "benchmark": "kitti_odometry_seq08_public_validation",
        "generated_by": "evaluation/scripts/run_m_gclo_kitti_seq08_validation.py",
        "scope": {
            "dataset": "KITTI Odometry sequence 08 full",
            "frames": on_data.get("num_frames"),
            "trajectory_length_m": on_data.get("trajectory_length_m"),
            "claim_limit": (
                "Public-sequence mechanism check on hilly KITTI seq08; not a substitute "
                "for dedicated off-road / multi-beam non-flat benchmarks such as MulRan."
            ),
        },
        "artifacts": {
        ...
        "conclusion": (
            "On public KITTI Odometry seq08 full, disabling the classified ground factor "
            "leaves translational RPE nearly unchanged "
            f"({mg_on['rpe_trans_pct']:.3f}% vs {mg_off['rpe_trans_pct']:.3f}%), "
            f"but whole-run ATE worsens by about {ate_rel:.1f}% "
            f"({mg_on['ate_m']:.2f} m vs {mg_off['ate_m']:.2f} m) and rotational RPE "
            f"rises by about {delta_block.get('rpe_rot_deg_per_m_relative_pct', 0):.1f}%. "
            "This matches the seq00/07 KITTI ablation pattern: multiple-ground constraints "
            "anchor global/attitude stability on hilly public driving data, even when local "
            "translational drift stays near KISS-ICP. Dedicated off-road / multi-beam non-flat "
            "benchmarks such as MulRan remain open before T0 promotion."
        ),
    }
...
def main() -> int:
    ...
    summary_json = BENCH_DIR / "m_gclo_kitti_seq08_validation_summary.json"
    ...
    if not ground_on_json.exists() or not ground_off_json.exists():
        raise SystemExit("missing seq08 validation artifacts; run without --summary-only")
