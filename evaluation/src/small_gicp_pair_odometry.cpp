...
void writeOutput(
    ...
    const std::vector<PairLog>& pairs, const std::vector<GTPose>& gt,
    const localization_zoo::small_gicp::SmallGICPParams& params,
  ...
  out << "  \"method\": \"small_gicp_pair_odometry\",\n";
  out << "  \"parameters\": {\n";
  out << "    \"voxel_resolution\": " << params.voxel_resolution << ",\n";
  out << "    \"k_neighbors\": " << params.k_neighbors << ",\n";
  out << "    \"max_correspondence_distance\": "
      << params.max_correspondence_distance << ",\n";
  out << "    \"max_correspondences\": " << params.max_correspondences << ",\n";
...
int main(int argc, char** argv) {
  if (argc < 4) {
    std::cerr << "Usage: small_gicp_pair_odometry <pcd_dir> <gt_csv> "
                 "<output_json> [max_frames] [--voxel-resolution X] "
                 "[--max-correspondence-distance X] [--max-step-translation X] "
                 "[--max-step-yaw-deg X] [--max-iterations N] "
                 "[--max-correspondences N] [--k-neighbors N]\n";
    return 1;
  }
  ...
  localization_zoo::small_gicp::SmallGICPParams params;
  ...
  for (int i = opt_begin; i < argc; ++i) {
    ...
    if (arg == "--voxel-resolution" || arg == "--target-voxel-size" ||
        arg == "--source-voxel-size") {
      ...
      params.max_iterations = std::stoi(needValue(arg));
    } else if (arg == "--max-correspondences") {
      params.max_correspondences = std::max(10, std::stoi(needValue(arg)));
    } else if (arg == "--k-neighbors") {
      params.k_neighbors = std::max(4, std::stoi(needValue(arg)));
    } else {
  ...
  localization_zoo::small_gicp::SmallGICPRegistration matcher(params);
  ...
  for (size_t i = 1; i < pcds.size(); ++i) {
    ...
    log.raw = result.transformation;
    log.converged = result.converged;
    log.correspondences = result.num_correspondences;
    log.iterations = result.num_iterations;
    log.rmse = std::sqrt(std::max(0.0, result.fitness));
    log.accepted =
        result.converged &&
        passesGate(result.transformation, max_step_translation, max_step_yaw_deg);

    if (log.accepted) {
      log.used = result.transformation;
    ...
    std::cout << "\r[SmallGICP-pair] " << i << "/" << pcds.size() - 1
