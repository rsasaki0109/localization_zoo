...
`indoor_kidnap_01/02` were localized against the dataset-provided
`map_indoor_hard.ply`. This required adding
...
The GT-free guard inside `pcd_dogfooding` is unsafe on these traces:
...
| **Total** | **2,183** | **2,163** | **16.065** |

The generated replay report counts 2,155 embedded wrong outputs because it
...
The post-hoc GT sequence verifier classifies both traces as `block_publish`:
2,985/3,076 raw frames are wrong and 1,480 transitions are unsafe jumps.

The default replay (`max_hold_frames=3`) still emits 9 wrong poses among 55
outputs, up to 3.843 m error. Setting `max_hold_frames=0` produces 45 outputs
with zero wrong poses (maximum 0.945 m) and suppresses all 2,985 wrong frames.
