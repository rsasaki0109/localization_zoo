...
bool projectLandmark(const Landmark& landmark, const Eigen::Matrix4d& pose,
                     const FastLivo2Params& params, Eigen::Vector2d* pixel) {
  const Eigen::Matrix3d rotation = pose.block<3, 3>(0, 0);
  const Eigen::Vector3d translation = pose.block<3, 1>(0, 3);
...
FastLivo2Params makeBaseParams() {
  FastLivo2Params params;
...
TEST(FastLivo2, VisualRefinementImprovesDegenerateSequence) {
  FastLivo2Params params = makeBaseParams();
  const auto landmarks = makeLandmarks();

  FastLivo2 pipeline(params);
  pipeline.setLandmarks(landmarks);

  FastLivo2Result last_result;
  ...
  const double raw_error = (last_result.front_end_state.trans - ground_truth).norm();
  const double corrected_error = (last_result.state.trans - ground_truth).norm();

  ASSERT_TRUE(last_result.initialized);
  EXPECT_TRUE(last_result.visual_used);
  EXPECT_GT(last_result.num_visual_factors, 5);
  EXPECT_LT(corrected_error, raw_error);
  EXPECT_LT(corrected_error, 1.0);
}

TEST(FastLivo2, SparseVisualInputFallsBackToFrontEnd) {
  FastLivo2Params params = makeBaseParams();
  params.min_visual_observations = 20;
  const auto landmarks = makeLandmarks();

  FastLivo2 pipeline(params);
  pipeline.setLandmarks(landmarks);

  FastLivo2Result result = pipeline.process(
      generateScan(makePose(0.0, 0.0, 1.0, 0.0)),
      std::vector<VisualObservation>{
          VisualObservation{landmarks.front().id, Eigen::Vector2d(320.0, 240.0)}});

  ASSERT_TRUE(result.initialized);
  EXPECT_FALSE(result.visual_used);
  EXPECT_EQ(result.num_visual_factors, 0);
}

TEST(FastLivo2, ClearResetsStateAndLandmarks) {
  FastLivo2Params params = makeBaseParams();
  const auto landmarks = makeLandmarks();
