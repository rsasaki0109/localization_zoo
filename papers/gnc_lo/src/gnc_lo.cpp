...
double GncLoPipeline::gncWeight(double residual, double truncation, double mu) {
  const double c2 = truncation * truncation;
  const double r2 = residual * residual;
  if (mu <= 0.0) return r2 <= c2 ? 1.0 : 0.0;  // μ→∞ 極限 (hard TLS)
  const double lower = mu / (mu + 1.0) * c2;
  const double upper = (mu + 1.0) / mu * c2;
  if (r2 <= lower) return 1.0;
  if (r2 >= upper) return 0.0;
  const double w = truncation / std::abs(residual) * std::sqrt(mu * (mu + 1.0)) - mu;
  return std::clamp(w, 0.0, 1.0);
}

GncLoPipeline::GncLoPipeline(const GncLoParams& params)
    : params_(params),
      local_map_(params.voxel_size, params.max_points_per_voxel) {}

std::vector<Eigen::Vector3d> GncLoPipeline::voxelDownsample(
...
std::vector<Eigen::Vector3d> GncLoPipeline::rangeFilter(
...
Eigen::Matrix4d GncLoPipeline::predict() const {
  if (delta_window_.empty()) return pose_;
  return pose_ * delta_window_.back();
}

Eigen::Matrix4d GncLoPipeline::runRegistration(
    const std::vector<Eigen::Vector3d>& source, const Eigen::Matrix4d& base,
    GncLoResult* result) {
  using Vec6 = Eigen::Matrix<double, 6, 1>;
  using Mat6 = Eigen::Matrix<double, 6, 6>;
  const double c = params_.gnc_truncation;

  Eigen::Matrix4d T = base;
  int last_corr = 0;
  double inlier_ratio = 0.0;
  double mu = params_.gnc_mu_init;  // <=0 なら初回に自動初期化
  bool mu_inited = (params_.gnc_mu_init > 0.0);
  ...
  for (int it = 0; it < params_.max_iterations; it++) {
    ...
    // 残差を集める。
    struct Item { Eigen::Matrix<double, 1, 6> Jr; double e; };
    std::vector<Item> items;
    items.reserve(source.size());
    double r_max = 0.0;
    for (size_t k = 0; k < source.size(); k++) {
      const auto& cc = corr[k];
      if (!cc.found || !cc.has_normal || cc.planarity < params_.planarity_threshold)
        continue;
      const double e = cc.normal.dot(src[k] - cc.anchor);
      Eigen::Matrix<double, 3, 6> J;
      J.block<3, 3>(0, 0) = -skew(src[k]);
      J.block<3, 3>(0, 3) = Eigen::Matrix3d::Identity();
      items.push_back({cc.normal.transpose() * J, e});
      r_max = std::max(r_max, std::abs(e));
    }
    if (static_cast<int>(items.size()) < 10) break;

    // 凸スタートの μ 自動初期化: μ⁰ = c̄² / (2 r_max² − c̄²)。
    if (!mu_inited) {
      const double denom = 2.0 * r_max * r_max - c * c;
      mu = denom > 1e-9 ? c * c / denom : 1e-3;
      mu = std::max(mu, 1e-3);
      mu_inited = true;
    }
    ...
    for (const auto& item : items) {
      const double w = gncWeight(item.e, c, mu);
      A += w * item.Jr.transpose() * item.Jr;
      b -= w * item.Jr.transpose() * item.e;
      w_sum += w;
    }
    inlier_ratio = w_sum / static_cast<double>(items.size());

    const Vec6 d = A.ldlt().solve(b);
    if (!d.allFinite()) break;
    T = expSE3(d) * T;
    last_corr = static_cast<int>(items.size());
    if (result) result->iterations = it + 1;

    mu *= params_.gnc_factor;  // 継続: 凸 → 非凸 TLS
...
GncLoResult GncLoPipeline::registerFrame(
