...
LioSam::LioSam(const LioSamParams& params)
    : params_(params),
...
Eigen::Matrix4d LioSam::latestOptimizedPoseMatrix() const {
  if (keyframes_.empty()) {
...
bool LioSam::shouldCreateKeyframe(const PoseState& pose) const {
  if (keyframes_.empty()) {
    return true;
  }
  ...
  const double rotation =
      rotationAngle(last.q.conjugate() * pose.q);
...
bool LioSam::tryAddLoopEdge(int loop_index, int current_index) {
...
LioSam::PoseState LioSam::relativePose(const PoseState& from,
                                       const PoseState& to) {
  PoseState relative;
...
LioSam::PoseState LioSam::matrixToPose(const Eigen::Matrix4d& transform) {
  PoseState pose;
...
Eigen::Matrix4d LioSam::poseToMatrix(const PoseState& pose) {
