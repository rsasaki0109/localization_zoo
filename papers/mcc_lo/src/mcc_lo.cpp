...
double MccLoPipeline::correntropyWeight(double residual, double sigma) {
  const double s = std::max(sigma, 1e-9);
  const double r = residual / s;
  // 相関エントロピー V=Σexp(−r²/2σ²) の半二次最適化で得られる Welsch 重み。
  return std::exp(-0.5 * r * r);
}

MccLoPipeline::MccLoPipeline(const MccLoParams& params)
    : params_(params),
      local_map_(params.voxel_size, params.max_points_per_voxel) {}

std::vector<Eigen::Vector3d> MccLoPipeline::voxelDownsample(
...
std::vector<Eigen::Vector3d> MccLoPipeline::rangeFilter(
...
Eigen::Matrix4d MccLoPipeline::predict() const {
  if (delta_window_.empty()) return pose_;
  return pose_ * delta_window_.back();
}

Eigen::Matrix4d MccLoPipeline::runRegistration(
  ...
  double sigma_used = params_.mcc_sigma;
  ...
  for (int it = 0; it < params_.max_iterations; it++) {
    ...
    // カーネルバンド幅 σ: Silverman の経験則 σ = 1.06·std(r)·N^(−1/5)。
    if (params_.mcc_adaptive_sigma) {
      const double n = static_cast<double>(items.size());
      const double mean = sum / n;
      const double var = std::max(sum_sq / n - mean * mean, 0.0);
      const double silverman = 1.06 * std::sqrt(var) * std::pow(n, -0.2);
      sigma_used = std::max(silverman, params_.mcc_sigma_floor);
    } else {
      sigma_used = params_.mcc_sigma;
    }
    ...
    for (const auto& item : items) {
      const double w = correntropyWeight(item.e, sigma_used);
      A += w * item.Jr.transpose() * item.Jr;
      b -= w * item.Jr.transpose() * item.e;
      w_sum += w;
    }
    mean_weight = w_sum / static_cast<double>(items.size());
...
MccLoResult MccLoPipeline::registerFrame(
