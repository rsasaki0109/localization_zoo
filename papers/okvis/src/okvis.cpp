...
Okvis::Okvis(const OkvisParams& params) : params_(params) {}

void Okvis::setLandmarks(const std::vector<Landmark>& landmarks) {
...
void Okvis::clearLandmarks() {
  landmarks_.clear();
  landmark_lookup_.clear();
}

OkvisResult Okvis::process(
  ...
  if (!initialized_) {
    ...
    FrameState frame;
    frame.raw_pose = current_state_;
    frame.optimized_pose = current_state_;
    if (params_.enable_visual_factors) {
      frame.observations = matchObservations(current_state_, visual_observations);
    }
    ...
    result.visual_used =
        static_cast<int>(window_frames_.back().observations.size()) >=
  ...
  FrameState frame;
  frame.raw_pose = predicted_pose;
  frame.optimized_pose = predicted_pose;
  if (params_.enable_visual_factors) {
    frame.observations = matchObservations(predicted_pose, visual_observations);
  }

  const int current_index = static_cast<int>(window_frames_.size());
  window_frames_.push_back(std::move(frame));
  result.keyframe_added = true;
  result.num_visual_factors =
      static_cast<int>(window_frames_.back().observations.size());
  ...
  if (params_.enable_imu_factors && current_index > 0) {
    ...
    edge.delta_q = (window_frames_[static_cast<size_t>(current_index - 1)]
                        .optimized_pose.q
                    .conjugate() *
                    predicted_pose.q)
                       .normalized();
    edge.delta_t =
        window_frames_[static_cast<size_t>(current_index - 1)].optimized_pose.q
            .conjugate() *
        (predicted_pose.t -
         window_frames_[static_cast<size_t>(current_index - 1)].optimized_pose.t);
  ...
  current_state_ = window_frames_.back().optimized_pose;
  result.q_w_curr = current_state_.q;
  result.t_w_curr = current_state_.t;
  result.v_w_curr = current_state_.v;
  result.num_keyframes = numKeyframes();
...
Eigen::Matrix4d Okvis::latestOptimizedPoseMatrix() const {
  if (window_frames_.empty()) {
    return Eigen::Matrix4d::Identity();
  }
  return poseToMatrix(window_frames_.back().optimized_pose);
}
...
bool Okvis::shouldCreateKeyframe(const PoseState& pose) const {
  if (window_frames_.empty()) {
    return true;
  }

  const PoseState& last = window_frames_.back().optimized_pose;
...
Okvis::PoseState Okvis::predictPose(
    const std::vector<imu_preintegration::ImuSample>& imu_samples,
    bool* imu_used) const {
...
std::vector<Okvis::MatchedObservation> Okvis::matchObservations(
...
bool Okvis::projectLandmark(const PoseState& pose,
                            const Eigen::Vector3d& landmark_world,
...
void Okvis::optimizeWindow() {
  if (window_frames_.size() < 2) {
    return;
  }

  std::vector<std::array<double, 4>> q_params(window_frames_.size());
  std::vector<std::array<double, 3>> t_params(window_frames_.size());
  for (size_t i = 0; i < window_frames_.size(); ++i) {
    const PoseState& pose = window_frames_[i].optimized_pose;
  ...
  if (params_.enable_visual_factors) {
    ...
    for (size_t i = 0; i < window_frames_.size(); ++i) {
      if (static_cast<int>(window_frames_[i].observations.size()) <
          params_.min_visual_observations) {
        continue;
      }

      for (const auto& observation : window_frames_[i].observations) {
        problem.AddResidualBlock(
  ...
  for (size_t i = 0; i < window_frames_.size(); ++i) {
    ...
    window_frames_[i].optimized_pose.q = q.normalized();
    window_frames_[i].optimized_pose.t = t;
    if (i == 0) {
      window_frames_[i].optimized_pose.v = current_state_.v;
      continue;
    }
    ...
    window_frames_[i].optimized_pose.v =
        (window_frames_[i].optimized_pose.t -
         window_frames_[i - 1].optimized_pose.t) /
        dt;
  }
}

void Okvis::pruneWindow() {
  while (static_cast<int>(window_frames_.size()) > params_.max_keyframes) {
...
Eigen::Matrix4d Okvis::poseToMatrix(const PoseState& pose) {
...
double Okvis::rotationAngle(const Eigen::Quaterniond& q) {
