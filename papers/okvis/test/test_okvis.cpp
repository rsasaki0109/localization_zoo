...
std::vector<Landmark> makeLandmarks() {
  ...
  for (double x = 3.0; x <= 24.0; x += 2.0) {
    for (double y : {-5.0, -2.5, 0.0, 2.5, 5.0}) {
      landmarks.push_back(
...
bool projectLandmark(const Landmark& landmark, const Eigen::Matrix4d& pose,
                     const OkvisParams& params, Eigen::Vector2d* pixel) {
...
OkvisParams makeBaseParams() {
  OkvisParams params;
  params.initial_velocity = Eigen::Vector3d(6.8, 0.0, 0.0);
  params.max_keyframes = 4;
...
TEST(OKVIS, LocalWindowImprovesImuPrediction) {
  OkvisParams params = makeBaseParams();
  const auto landmarks = makeLandmarks();

  Okvis pipeline(params);
  pipeline.setLandmarks(landmarks);

  OkvisResult last_result;
  ...
  EXPECT_LE(last_result.num_keyframes, params.max_keyframes);
  EXPECT_LT(optimized_error, raw_error);
  EXPECT_LT(optimized_error, 0.5);
}

TEST(OKVIS, WindowBoundsKeyframeCount) {
  OkvisParams params = makeBaseParams();
...
TEST(OKVIS, ClearResetsWindowAndKeepsLandmarksUntilExplicitlyCleared) {
  OkvisParams params = makeBaseParams();
  const auto landmarks = makeLandmarks();
