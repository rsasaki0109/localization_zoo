...
TEST(PointLio, TracksShortSequence) {
  ...
  params.max_correspondences = 200;

  PointLio pipeline(params);
  PointLioResult last_result;
  ...
  EXPECT_GT(last_result.map_points, 0u);
  EXPECT_LT((last_result.state.position - Eigen::Vector3d(3.5, 0.5, 0.0)).norm(),
            4.0);
}

TEST(PointLio, SlidingWindowMapBoundsKeyframeCount) {
  PointLioParams params;
...
TEST(PointLio, ClearResetsStateAndMap) {
  PointLio pipeline;
  for (int frame = 0; frame < 4; ++frame) {
    const std::vector<Eigen::Vector3d> scan =
        generateScan(makePose(0.8 * frame, 0.1 * frame, 0.0));
    const std::vector<ImuSample> imu_samples =
        frame == 0 ? std::vector<ImuSample>()
                   : makeImuSamples((frame - 1) * kScanPeriod, frame * kScanPeriod,
                                    0.0, 0.0);
