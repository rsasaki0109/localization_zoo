...
std::vector<Eigen::Vector3d> makeCloud(std::mt19937& rng) {
  ...
  for (int i = 0; i < 1200; ++i) {
    points.emplace_back(xy(rng), 14.0 + noise(rng), z(rng));
  }
  for (int i = 0; i < 1200; ++i) {
    points.emplace_back(-14.0 + noise(rng), xy(rng), z(rng));
  }
  return points;
}
...
TEST(SmallGICP, TargetSetup) {
  std::mt19937 rng(42);
  SmallGICPRegistration reg;
  reg.setTarget(makeCloud(rng));
  EXPECT_GT(reg.targetSize(), 0u);
}

TEST(SmallGICP, SmallTranslation) {
  std::mt19937 rng(42);
  ...
  T_gt(0, 3) = 0.6;
  T_gt(1, 3) = -0.2;
  const auto source = transform(target, T_gt);

  SmallGICPParams params;
  params.voxel_resolution = 1.0;
  params.max_iterations = 18;
  params.max_correspondences = 192;
  ...
  EXPECT_LT(trans_err, 0.6);
  EXPECT_GT(result.num_correspondences, 40);
}

TEST(SmallGICP, RotationAndTranslation) {
  std::mt19937 rng(7);
  const auto target = makeCloud(rng);
  ...
  params.max_correspondences = 256;
  SmallGICPRegistration reg(params);
  reg.setTarget(target);
