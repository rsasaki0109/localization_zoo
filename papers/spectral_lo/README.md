...
`--spectral-lo-bev-range 35 --spectral-lo-translation-refinement-iters 1` で
**3.94%** / **27 m**、**~7 FPS**。1 回の residual translation POC refinement により、
seq07 は **4.13% → 3.94%** に下がった。以前の 4.05% / 7.17% 行は 256 BEV の
高速 profile で、さらに古い 10.73% / 10.23% 行は current→ref de-rotation を
逆符号でかけていたため、yaw を含む前進時に並進 POC が崩れていた。

**正直な所見**: ICP-free な BEV 位相相関は反復不要で発散しないが、精度と速度は
BEV 解像度に強く依存する。512 BEV は 256 BEV より drift を下げる一方で ~10 FPS
まで落ち、residual translation refinement を入れると seq07 は ~7 FPS まで落ちる。
まだ **粗い**: (1) BEV セル量子化が並進分解能の下限となり
