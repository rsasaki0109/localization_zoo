...
published row を `--spectral-lo-bev-range 45` に更新した。seq00 RPE **4.05%** /
ATE **61 m**、seq07 **7.17%** / **34 m**、**~29–30 FPS**。以前の
10.73% / 10.23% 行は current→ref de-rotation を逆符号でかけていたため、
yaw を含む前進時に並進 POC が崩れていた。
...
fast-but-coarse 結果で、機構の正しさ (FFT・Fourier-Mellin・POC と yaw つき前進) は
ユニットテストで担保している。
