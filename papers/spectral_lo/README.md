...
- `bev_range` (既定 60 m): BEV 半径。セル = 2*bev_range/bev_size ≈ 0.47 m。
...
KITTI full (no GT seed) では seq00 RPE **12.03%** / seq07 **13.67%**、ATE 113/47 m。
**最速級の ~14 FPS** だが精度は低い。

**正直な所見**: ICP-free な BEV 位相相関は反復不要で高速・発散しない (DiLO の 18%
ほど崩れない) が、**粗い**: (1) BEV セル量子化 (~0.47 m) が並進分解能の下限となり
