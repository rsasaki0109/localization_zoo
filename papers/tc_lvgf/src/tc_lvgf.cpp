...
TcLvgfPipeline::TcLvgfPipeline(const TcLvgfParams& params) : params_(params) {}

void TcLvgfPipeline::reset() {
  frame_count_ = 0;
  pose_.setIdentity();
...
size_t TcLvgfPipeline::mapSize() const {
  return static_cast<size_t>(std::count_if(
      map_points_.begin(), map_points_.end(),
      [](const MapPoint& p) { return !p.removed; }));
}

size_t TcLvgfPipeline::lineMapSize() const {
  return static_cast<size_t>(std::count_if(
      map_lines_.begin(), map_lines_.end(),
      [](const MapLine& l) { return !l.removed; }));
}

Eigen::Vector3i TcLvgfPipeline::voxelKey(const Eigen::Vector3d& p) const {
...
int TcLvgfPipeline::imageIndex(int row, int col) const {
  col = ((col % params_.image_width) + params_.image_width) % params_.image_width;
  return row * params_.image_width + col;
}

bool TcLvgfPipeline::projectToSpherical(const Eigen::Vector3d& point, int* row,
...
std::vector<Eigen::Vector3d> TcLvgfPipeline::rangeFilter(
...
std::vector<Eigen::Vector3d> TcLvgfPipeline::voxelDownsample(
...
std::vector<TcLvgfPipeline::RangeCell> TcLvgfPipeline::buildRangeImage(
    const std::vector<Eigen::Vector3d>& points) const {
...
bool TcLvgfPipeline::fitLine(const std::vector<Eigen::Vector3d>& points,
...
bool TcLvgfPipeline::fitPlane(const std::vector<Eigen::Vector3d>& points,
...
std::vector<LineFeature> TcLvgfPipeline::extractLidarLines(
...
std::vector<LineFeature> TcLvgfPipeline::extractVisualLines(
    const std::vector<RangeCell>& image) const {
  std::vector<LineFeature> lines;
...
std::vector<LineFeature> TcLvgfPipeline::fuseLines(
...
FrameFeatures TcLvgfPipeline::extractFeatures(
...
TcLvgfPipeline::PlaneMatch TcLvgfPipeline::queryPlane(
...
TcLvgfPipeline::LineMatch TcLvgfPipeline::queryLine(
...
Eigen::Matrix4d TcLvgfPipeline::align(const FrameFeatures& features,
                                      const Eigen::Matrix4d& initial_guess,
                                      TcLvgfResult* result) const {
  ...
  for (int iter = 0; iter < params_.max_icp_iterations; ++iter) {
    ...
    for (const auto& line : features.fused_lines) {
      ...
      const double w = params_.line_weight * m.weight * line.weight * robust;
      A += w * J.transpose() * J;
      b -= w * J.transpose() * r;
      residual_sum += r.norm();
      ++residual_count;
      ...
      const double wd = params_.direction_weight * line.weight * robust;
    ...
    const int total = plane_count + line_count;
...
void TcLvgfPipeline::addPoint(const Eigen::Vector3d& point, int frame_index) {
...
void TcLvgfPipeline::addLine(const LineFeature& line, int frame_index) {
...
void TcLvgfPipeline::updateMap(const FrameFeatures& features,
...
void TcLvgfPipeline::pruneFar(const Eigen::Vector3d& center) {
...
TcLvgfResult TcLvgfPipeline::registerFrame(
