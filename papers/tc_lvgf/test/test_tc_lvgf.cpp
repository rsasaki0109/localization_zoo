...
TEST(TcLvgf, ProjectsToSphericalImage) {
  TcLvgfPipeline pipe(testParams());
  int row = -1;
  int col = -1;
  double range = 0.0;
...
TEST(TcLvgf, ExtractsAndFusesGeometricLines) {
  std::mt19937 rng(8);
  TcLvgfPipeline pipe(testParams());
...
TEST(TcLvgf, TracksTranslationSequence) {
  std::mt19937 rng(9);
...
TEST(TcLvgf, FallsBackToLidarWhenVisualLinesAreSparse) {
  TcLvgfParams params = testParams();
  params.min_visual_lines = 50;
  TcLvgfPipeline pipe(params);
