...
using namespace localization_zoo::tc_mvlo;

namespace {

TcMvloParams testParams() {
  TcMvloParams params;
...
TEST(TcMvlo, AppliesMonocularVisualLidarProfile) {
  const auto p = TcMvloPipeline::makeBackendParams(testParams());
  EXPECT_GT(p.visual_point_weight, 0.0);
  EXPECT_GT(p.line_weight, 0.0);
  EXPECT_GT(p.visual_line_bonus, 1.0);
}

TEST(TcMvlo, ExtractsPointAndLineFeatures) {
  std::mt19937 rng(18);
  TcMvloPipeline pipe(testParams());
...
TEST(TcMvlo, TracksShortTranslation) {
  std::mt19937 rng(19);
