...
struct VLoam15Params {
  opl_lvio::OplLvioParams backend;
};

struct VLoam15Result {
  Eigen::Matrix4d pose = Eigen::Matrix4d::Identity();
  opl_lvio::OplLvioResult backend;
};

class VLoam15Pipeline {
 public:
  explicit VLoam15Pipeline(const VLoam15Params& params = VLoam15Params());
  ...
  VLoam15Result registerFrame(const std::vector<Eigen::Vector3d>& frame);
