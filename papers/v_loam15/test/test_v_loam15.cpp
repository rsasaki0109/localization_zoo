...
using namespace localization_zoo::v_loam15;

namespace {

VLoam15Params testParams() {
  VLoam15Params params;
...
TEST(VLoam15, AppliesVisualBootstrapProfile) {
  const auto p = VLoam15Pipeline::makeBackendParams(testParams());
  EXPECT_GT(p.visual_point_weight, 0.0);
  EXPECT_EQ(p.line_weight, 0.0);
  EXPECT_GT(p.motion_prior_weight, 0.0);
}

TEST(VLoam15, ExtractsRangeImageVisualFeatures) {
  std::mt19937 rng(12);
  VLoam15Pipeline pipe(testParams());
...
TEST(VLoam15, TracksShortTranslation) {
  std::mt19937 rng(13);
