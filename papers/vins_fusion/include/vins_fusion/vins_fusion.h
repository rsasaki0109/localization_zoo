...
struct VinsFusionParams {
  ...
  double keyframe_translation_threshold = 0.25;
...
class VinsFusion {
public:
  explicit VinsFusion(const VinsFusionParams& params = VinsFusionParams());

  void setLandmarks(const std::vector<Landmark>& landmarks);
  void clearLandmarks();

  VinsFusionResult process(
